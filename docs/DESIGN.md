...
during the mutation window prevents the deployment run from becoming a trusted release
record. This separates safe application rollback from forward-only reconciliation of
current infrastructure security controls.
...
- CodeQL SAST;
- root/web/Lambda fail-closed SCA, exact npm-override verification, and an
  infra-only checksum-pinned, receipt-bound exact-path compensation for the
  immutable `aws-cdk-lib@2.262.1` bundled GHSA; PR dependency review admits
  only that same GHSA until an upstream fixed bundle is available;
...
- a daily rescan that uses a dedicated runtime-read OIDC role to bind the exact live
  `Archon-production` release to its successful deployment run and immutable evidence,
  then verifies current ECS task-definition/running image digests, Lambda ZIP/content/
  configuration digests, every latest versioned KMS-encrypted SPA object, the embedded CI
  run, GitHub artifact digests, and the exact retained subjects. It repeats the full AWS
  byte observation after scanning, revalidates the sealed observer receipt before each
  read-only OIDC boundary, and performs one final current-ref/whole-snapshot comparison
  immediately before the production provenance attestation. It then independently reads
  the latest CodeQL and workflow-security runs for the exact deployed historical source
  SHA twice and requires both snapshots to equal the sealed source-gate receipt, without
  requiring that source SHA to remain current `master`; the exact CI-run/SHA manual rescan
  path remains available, and staging/production reject attestations older than 24 hours;
- a separate scheduled production-posture workflow that requires exact-SHA green security
  gates before read-only OIDC, then proves termination protection, no CloudFormation
  drift, a confirmed alarm subscription, and unchanged post-check state for the registry,
  platform, and edge stacks;
