...
workflow is source-complete, but it must remain blocked until an eligible
individual has actually submitted the feedback form and a second person has
privately verified the authenticated confirmation.
...
“not personal data.” The signed support also retains the independent reviewer’s
...
confirmation/reference that an independent reviewer can verify, leave the
bonus blocked.
...
- required individual reviewers;
- prevent self-review enabled;
- one custom deployment policy of type `branch` allowing only `master`; and
- no secrets.

The reviewer must differ from both the workflow actor and triggering actor. The
reviewer privately verifies all of the following before approving:
...
2. `review` runs behind the protected environment. It verifies the exact
   independent approval through the GitHub API, redownloads and revalidates the
...
- an independent configured reviewer can privately verify the form,
