...
for library in \
  "${script_dir}/aws-foundation-cloud-runtime-publisher-policy-migration-common.sh" \
  "${script_dir}/aws-foundation-cloud-runtime-publisher-policy-migration-authorization.sh" \
  "${script_dir}/aws-foundation-cloud-runtime-publisher-policy-migration-state.sh"; do
  test -f "${library}"
  test ! -L "${library}"
...
write_receipt() {
  ...
  jq -cnS \
    ...
    --arg state "${expected_state}" '
      {
        schemaVersion: "archon.aws-foundation-cloud-runtime-publisher-policy-migration-receipt/v1",
        authorization: {
          absenceReadCount: 3,
          state: "absent"
        },
        ...
        policy: {
          ...
          name: "archon-aws-foundation-identity",
        ...
        migration: {purpose: "cloud-runtime-publisher-and-cloud-trial-role-authorization"}
      }
  ...
  jq -cnS \
    ...
    --arg result "${result}" '
      {
        schemaVersion: "archon.aws-foundation-cloud-runtime-publisher-policy-migration-attestation/v1",
