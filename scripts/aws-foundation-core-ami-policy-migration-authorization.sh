    ...
    --argjson actions "${actions}" '
      {
        ...
        Statement: [
          {
            Sid: "OperateExactCoreAmiControlPolicy",
            Effect: "Allow",
            Action: $actions,
            Resource: $policyArn,
            Condition: {
...
wait_for_temp_digest() {
  ...
  for ((attempt = 1; attempt <= RETRY_ATTEMPTS; attempt++)); do
    if aws iam get-role-policy \
      ...
      if jq -e '
        ...
        .PolicyName == "archon-foundation-core-ami-control-policy-migration" and
        (.PolicyDocument | type) == "object"
      ' "${output}" >/dev/null; then
        observed_sha="$(iam_policy_sha "${output}" '.PolicyDocument')" || {
          fail "Unable to canonicalize temporary Core AMI control-policy migration authorization"
          return 1
        }
  ...
  fail "The temporary Core AMI control-policy migration authorization was not canonically readable"
  return 1
}
...
install_temp_policy() {
  ...
  expiry_epoch="$(date --utc --date="${expires_at}" '+%s')" || {
    fail "Core AMI control-policy migration expiry is invalid"
    return 1
  }
  remaining=$((expiry_epoch - now_epoch))
  ((remaining >= 1140 && remaining <= 1200)) || {
    fail "Core AMI control-policy migration authorization TTL is not exactly bounded to twenty minutes"
    return 1
  }
  ...
  now_epoch="$(date --utc '+%s')"
  expiry_epoch="$(date --utc --date="${expires_at}" '+%s')" || {
    fail "Core AMI control-policy migration expiry became invalid before authorization"
    return 1
  }
  remaining=$((expiry_epoch - now_epoch))
  ((remaining >= 1140 && remaining <= 1200)) || {
    fail "Core AMI control-policy migration authorization TTL changed before installation"
    return 1
  }
  if ! aws iam put-role-policy \
    ...
    fail "Unable to install the temporary Core AMI control-policy migration authorization"
    return 1
  fi
...
verify_live_temp_policy() {
  ...
  safe_aws "Unable to read the temporary Core AMI control-policy migration authorization" "${observed}" \
    iam get-role-policy \
    --role-name "${RECOVERY_ROLE_NAME}" \
    --policy-name "${TEMP_POLICY_NAME}" \
    --output json
  ...
  first_expiry="$(
    jq -er '.PolicyDocument.Statement[] |
      select(.Sid == "OperateExactCoreAmiControlPolicy") |
  ...
  test "${observed_sha}" = "${expected_document_sha}" || {
    fail "The live temporary Core AMI control-policy migration authorization differs"
    return 1
  }
  ...
  ((remaining >= minimum_remaining && remaining <= 1200)) || {
    fail "Temporary Core AMI control-policy migration authorization has insufficient lifetime"
    return 1
  }
