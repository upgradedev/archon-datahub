    ...
    --argjson actions "${actions}" '
      {
        ...
        Statement: [
          {
            Sid: "OperateExactFoundationIdentityPolicy",
            Effect: "Allow",
            Action: $actions,
            Resource: $policyArn,
            Condition: {
  ...
  for ((attempt = 1; attempt <= RETRY_ATTEMPTS; attempt++)); do
    if aws iam get-role-policy \
      ...
      if jq -e '
        ...
        .PolicyName == "archon-foundation-cloud-runtime-publisher-identity-policy-migration" and
...
verify_live_temp_policy() {
  ...
  first_expiry="$(
    jq -er '.PolicyDocument.Statement[] |
      select(.Sid == "OperateExactFoundationIdentityPolicy") |
