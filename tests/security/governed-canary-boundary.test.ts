...
test("write and inverse are separately approved and share a non-cancelling lock", () => {
  ...
  assert.doesNotMatch(prepare, /secrets\.CANARY_DATAHUB_(READ|WRITE)_TOKEN/);
  assert.doesNotMatch(execute, /secrets\.CANARY_DATAHUB_(READ|WRITE)_TOKEN/);
  assert.match(rollback, /secrets\.CANARY_DATAHUB_READ_TOKEN/);
  assert.match(rollback, /secrets\.CANARY_DATAHUB_WRITE_TOKEN/);
...
test("interrupted recovery authenticates the exact immutable source before secrets", () => {
  ...
  assert.match(recovery, /secrets\.CANARY_DATAHUB_READ_TOKEN/);
  assert.match(recovery, /secrets\.CANARY_DATAHUB_WRITE_TOKEN/);
...
test("AWS read roles are scoped to the lean Judge stack and promotion stays independent", () => {
